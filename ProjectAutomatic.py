...
class ProjectAutomatic():
    ...
    def export_branches_pdf(self,array_project_id, directory):
        with open(directory + 'relatorio.csv', 'a+',) as csvfile:
            fields = ['Branch','Sistema','Autor','Squad', 'Criação','Ultima alteração','Tipo Branch','Quantidade de Commits', 'Commits atrás da master', 'Commits a frente da master', 'Diff com a master','Data merge']
            writer = csv.DictWriter(csvfile, fieldnames=fields, dialect='excel')
            reader = csv.DictReader(csvfile, fieldnames=fields)
            writer.writeheader()
            for project_id in array_project_id:
                try:
                    branches = self.get_project_branches(project_id)
                    project  = self.get_name(project_id)
                    merges   = self.get_project_merges(project_id)
                    
                    print('Gerando relatório do pacote '+str(project.name)+' com ID '+str(project_id))
                    ...
                    for branch in branches:
                        ...
                        if branch.name.find('master') == 0 or branch.name.find('release') == 0 or branch.name.find('develop') == 0 or branch.name.find('hml') == 0:
                            continue
                        else:
                            try:
                    ...
                    print("Erro ao gerar relatorio do projeto "+str(project.name)+" com ID: "+str(project_id))
    ...
    def branch_type(self, branch):
        if branch.find("feature") == 0:
            return "feature"
        if branch.find("hotfix") == 0:
            return "hotfix"
        if branch.find("release") == 0:
            return "release"
